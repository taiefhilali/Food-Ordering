...
import { faTrash, faBan, faList } from '@fortawesome/free-solid-svg-icons';
...
type User = {
    ...
    email: string;
    userType: string;
    restaurants: Restaurant[]; // Add this line to include restaurants

};

const VendorsDisplay: React.FC = () => {
    const [users, setUsers] = useState<User[]>([]);
    const [expandedUserId, setExpandedUserId] = useState<string | null>(null); // Track the currently expanded user
    ...
    const fetchUsers = async () => {
        try {
            ...
            const response = await axios.get('http://localhost:7000/api/my/user/vendors', {
                headers: {
            ...
            const usersWithRestaurants = await Promise.all(
                response.data.map(async (user: User) => {
                    const restaurantsResponse = await axios.get(
                        `http://localhost:7000/api/my/restaurant/${user._id}/restaurants`,
                        {
                            headers: {
                                Authorization: `Bearer ${token}`,
                            },
                    ...
                    return { ...user, restaurants: restaurantsResponse.data };
                })
            );
                        ...
                        <li key={user._id} className="user-card">
                            ...
                            <div className="user-card-content">
                                ...
                                <div className="user-info">
                                    ...
                                    {user.blocked && <span className="blocked-badge">Blocked</span>}
                                </div>
                                <button onClick={() => toggleRestaurants(user._id)}>
                                    <FontAwesomeIcon icon={faList} /> Restaurants
                                </button>
                                            ...
                                            <ul>
                                                {user.restaurants.map((restaurant) => (
                                                    <li key={restaurant._id}>
                                                        ...
                                                        <p>{restaurant.isAvailable ? 'Available' : 'Not Available'}</p>
                                                    </li>
                                                ))}
                                            </ul>
                                        ) : (
